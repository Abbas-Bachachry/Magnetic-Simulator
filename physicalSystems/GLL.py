...
@jit(nopython=True, nogil=True)
def llg_forc(no, hs, step, moment_vectors_history, jacobian, strayfield, no_particles, easy_axis_unit_vectors, hk,
             volumes, moment_and_residual, ms, matrix2, max_iteration, dumping_factor):
    ...
    :param np.ndarray strayfield: here the stray field array is np.zeros([self.data.no_particles, 3]); added only because numba can't creat 2dimensional array within parallel mode
    :param int no_particles: number of particles
    ...
    :param np.ndarray moment_and_residual: this is np.zeros([self.data.no_particles, 4]); added only because numba can't creat 2dimensional array within parallel mode
    ...
    for i in prange(no):
        ha = hs - i * step
        matrix2[i] = llg_curve(no, i, ha, step, moment_vectors_history, jacobian, strayfield,
                               no_particles, easy_axis_unit_vectors, hk, volumes, moment_and_residual,
                               ms, max_iteration, dumping_factor)
    return matrix2


@jit(nopython=True, nogil=True)
def llg_curve(no, i, ha, step, momhis, jacob, strayfield, no_particles, easy, hk, vol, moment_and_residual, ms,
              maxiter, f):
    ...
    """
    residual = np.zeros(no_particles)
    ...
    for j in prange(i + 1):
        ...
        for l in range(maxiter):
            m = np.zeros(3 * num_particles)
            h = np.zeros(3 * num_particles)
            m[0::3] = moment_unit_vectors[:, 0] * vol * ms
            m[1::3] = moment_unit_vectors[:, 1] * vol * ms
            m[2::3] = moment_unit_vectors[:, 2] * vol * ms
            h[:] = (jacob @ m)[:]
            strayfield[:, 0] = h[::3].T
            strayfield[:, 1] = h[1::3].T
            strayfield[:, 2] = h[2::3].T
            moment_reference_wave = llg_mt(hb, moment_unit_vectors, strayfield, easy, hk, f, moment_and_residual)
            for k in range(num_particles):
                w = moment_reference_wave[k]
                if not np.isnan(w[0]):
...
@jit(nopython=True, nogil=True)
def llg_mt(h, moment_vector, stray_vector, ea_vector, hk, f, moment_and_residual):
    ...
    :param stray_vector: an array containing the stray filed with shape of (number of particles, 3)
    ...
    :param moment_and_residual: this is np.zeros([self.data.no_particles, 4]); added only because numba can't creat 2dimensional array within parallel mode
    ...
    for k in prange(moment_vector.shape[0]):
        ...
        effective_filed = applied_field + stray_vector[k] + hk[k] * mz * ea_vector[k]
        ...
        # Set global residual wave to magnitude of residual vector
        moment_and_residual[k, 3] = (torque_vector[0] ** 2 + torque_vector[1] ** 2 + torque_vector[2] ** 2) ** 0.5
        # Calculate change in moment direction
        ...
        mag = (moment_vector[k, 0] ** 2 + moment_vector[k, 1] ** 2 + moment_vector[k, 2] ** 2) ** 0.5
        moment_vector[k] /= mag
        moment_and_residual[k, :3] = moment_vector[k]
